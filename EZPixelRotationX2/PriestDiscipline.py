...
子类化 RotationEngine 实现
"""

from rich import print
from EZPixelRotationX2 import (
    RotationEngine,
    AttrDict,
    SliderConfig,
    ComboConfig,
...
class PriestDisciplineEngine(RotationEngine):
    ...
    只需覆盖：
    - self.config: 添加特有配置项（在 _setup_config 中）
    - self.macros: 设置技能到按键的映射（在 _setup_macros 中）
    - main_rotation(): 实现主循环逻辑
    """
    ...
    def _setup_config(self):
        """设置戒律牧师特有配置"""
        # 自我治疗阈值
        self.config.add_item(SliderConfig(
        ...
        # 驱散逻辑
        self.config.add_item(ComboConfig(
            key="dispel_logic",
            label="驱散逻辑",
    ...
    def calculate_party_health_score(self, data: AttrDict) -> list[AttrDict]:
        """计算小队数据 - 内部辅助方法
        ...
        dispel_type_list = self.config.dispel_types  # 驱散类型：可驱散的debuff类型集合

        for unit in temp_members:
            unit_role = unit.status.unit_role
            unit_health = unit.status.unit_health
            unit_damage_absorbs = unit.status.unit_damage_absorbs   # 伤害吸收盾，吸收承受伤害的buff
            unit_heal_absorbs = unit.status.unit_heal_absorbs        # 治疗吸收盾，吸收治疗效果的debuff
            base_missing = 100.0 - unit_health
            unit_heal_deficit = base_missing + unit_heal_absorbs  # 治疗缺口，即治疗效果的缺口，用于计算治疗倾向

            if unit_role == "TANK":
                # TANK 忽略 20% 血上限的缺口（80% 血量视为满血）
                unit_heal_deficit = max(base_missing - self.config.tank_deficit_ignore_pct, 0.0) + unit_heal_absorbs  # TANK缺口忽略：把一定血量缺口视为满血

            unit_health_score = unit_health + unit_damage_absorbs - unit_heal_deficit  # 健康分数，用于计算治疗倾向

            if unit_role == "TANK":
                unit_health_score = unit_health_score * self.config.tank_health_score_mul            # TANK健康分数系数：提高坦克治疗优先级

            if unit_role == "HEALER":
                unit_health_score = unit_health_score * self.config.healer_health_score_mul            # HEALER健康分数系数：降低治疗优先级

            unit_dict = AttrDict({
                "unitToken": unit.unitToken,  # 目标ID
                "unit_class": unit.status.unit_class,  # 目标职业
                "unit_role": unit_role,  # 目标职责
                "unit_health": unit_health,  # 目标当前生命值
                "unit_damage_absorbs": unit_damage_absorbs,  # 伤害吸收盾，吸收承受伤害的buff
                "unit_heal_absorbs": unit_heal_absorbs,  # 治疗吸收盾，吸收治疗效果的debuff
                "unit_heal_deficit": unit_heal_deficit,  # 治疗缺口，即治疗效果的缺口，用于计算治疗倾向
                "unit_health_score": unit_health_score,  # 健康分数，用于计算治疗倾向
                "shield_remaining": unit.aura.buff["真言术：盾"].remaining or 0.0,  # 真言术：盾 剩余时间
                "atonement_remaining": unit.aura.buff["救赎"].remaining or 0.0,  # 救赎 剩余时间
                "fortitude_remaining": unit.aura.buff["真言术：韧"].remaining or 0.0,  # 真言术：韧 剩余时间
                "dispel_list": [debuff.type for debuff in unit.aura.debuff_sequence if debuff.type in dispel_type_list],  # 可驱散debuff类型列表
                "damage_debuff_count": len([debuff for debuff in unit.aura.debuff_sequence if debuff.title in high_damage_debuff]),  # 高伤害debuff数量
                "unit_in_range": unit.status.unit_in_range,
                "selectd": unit.status.selectd,

            })
            party_members.append(unit_dict)

        party_members.sort(key=lambda x: x.unit_health_score, reverse=False)  # 按健康分数排序，从低到高

        return party_members

    def main_rotation(self, data: AttrDict) -> None:
        """戒律牧师主循环 - 覆盖父类方法
        ...
        # 治疗缺口大于阈值的的人数
        need_heal_member = [unit for unit in in_range_member if unit.unit_heal_deficit > self.config.need_heal_deficit_threshold]  # 最小治疗缺口：超过该缺口才算需要治疗
        ...
        # 在苦修为攻守兼备的情况下，苦修CD好了就用。
        # 在苦修为仅治疗的情况下，队伍必须有任意人血量低于90%，才会用。
        # "攻守兼备", "仅治疗"
        ...
        if (spell["苦修"].charge >= 1) and gcd_ready and (penance_logic_1 or penance_logic_2):
            if len(need_heal_member) <= int(self.config.penance_heal_max_targets) and len(need_heal_member) > 0:  # 进攻苦修群抬目标数：超过则倾向打怪
                return self.cast(need_heal_member[0].unitToken, "苦修")
            # 否则给怪，没痛不给
            if auto_target:
                return self.cast(auto_target.unitToken, "苦修")

        # 苦修没CD的时候，对25%以上缺口的人用快速治疗
        can_cast_flash = player_is_stand or player.aura.buff["圣光涌动"]
        if in_range_member and (in_range_member[0].unit_heal_deficit > self.config.flash_heal_deficit_threshold) and gcd_ready and can_cast_flash:  # 快速治疗阈值：缺口超过则快速治疗
            return self.cast(in_range_member[0].unitToken, "快速治疗")

        # 用恳求补救赎
        # plea_logic_1 plea_logic为"血量大于缺口无救赎" and   存在血量大于缺口且无救赎的人
        ...
        # plea_logic_2  plea_logic为"任意情况补救赎" and 存在任意无救赎的人
        ...
        # plea_logic_3  plea_logic为"不使用" , 这个逻辑不需要
