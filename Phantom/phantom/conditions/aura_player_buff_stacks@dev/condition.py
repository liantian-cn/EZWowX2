...
Description:
    ...
    无光环或解码异常返回 0.0；ValueBarBackplate 仅提供背板，不创建默认填充。
Key Variables:
    max_value: 层数显示上限。
Change Log:
...
class Plugin(Condition):
    ...
    config_defaults: ClassVar[Mapping[str, object]] = {"min_value": 0, "width": 2, "player_only": True}

    def __init__(self, args: dict[str, object]) -> None:
        Fields(frozenset({"aura_ids", "max_value"}), frozenset({"min_value", "width", "player_only"})).validate(args, "plugin_args")
    ...
    def template_parameters(self) -> dict[str, str]:
        return {"aura_ids": ", ".join(str(value) for value in self.aura_ids), "max_value": str(self.max_value), "aura_filter": '"HELPFUL|PLAYER"' if self.player_only else '"HELPFUL"'}
